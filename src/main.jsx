...
function App() {
  ...
  const exportConversation = (format) => { if (!activeConversation?.messages.length) return showToast('Start a chat before exporting.'); const title = activeConversation.title || 'Adesh conversation'; const content = [`# ${title}`, '', ...activeConversation.messages.map((message) => `## ${message.role === 'assistant' ? 'Adesh' : 'You'}\n\n${message.content}`)].join('\n\n'); if (format === 'print') { const printWindow = window.open('', '_blank', 'width=800,height=700'); if (!printWindow) return showToast('Allow pop-ups to print this chat.'); printWindow.document.title = title; printWindow.document.body.style.cssText = 'font: 16px/1.6 system-ui; max-width: 760px; margin: 40px auto; white-space: pre-wrap;'; printWindow.document.body.textContent = content; printWindow.print(); return; } const blob = new Blob([content], { type: 'text/markdown;charset=utf-8' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `${title.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '').toLowerCase() || 'adesh-chat'}.md`; link.click(); URL.revokeObjectURL(url); showToast('Chat downloaded'); };
