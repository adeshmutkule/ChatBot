...
function App() {
  ...
  const handleFiles = (event) => { Array.from(event.target.files || []).slice(0, 3).forEach((file) => { const isText = file.type.startsWith('text/') || file.name.endsWith('.md'); const reader = new FileReader(); reader.onload = () => { const result = String(reader.result); const data = result.includes(',') ? result.split(',')[1] : btoa(unescape(encodeURIComponent(result))); setAttachments((current) => [...current, { name: file.name, mimeType: file.type || 'text/plain', data, content: isText ? result : '', previewUrl: result.startsWith('data:image/') ? result : '' }]); }; if (isText) reader.readAsText(file); else reader.readAsDataURL(file); }); event.target.value = ''; };
  ...
  const exportChat = (type) => { const body = [`Adesh conversation: ${activeConversation?.title || 'New conversation'}`, `Exported: ${new Date().toLocaleString()}`, '', ...messages.map((message) => `${message.role === 'assistant' ? 'Adesh' : 'You'} (${formatTime(message.createdAt)}):\n${message.content}`)].join('\n\n'); if (type === 'pdf') { const printWindow = window.open('', '_blank'); if (!printWindow) return showToast('Allow popups to export PDF.'); printWindow.document.write(`<title>${activeConversation?.title || 'Adesh chat'}</title><style>body{font:14px system-ui;max-width:760px;margin:40px auto;white-space:pre-wrap;line-height:1.6}</style><h1>${(activeConversation?.title || 'Adesh chat').replace(/</g, '&lt;')}</h1><pre>${body.replace(/</g, '&lt;')}</pre>`); printWindow.document.close(); printWindow.print(); return; } const blob = new Blob([body], { type: type === 'md' ? 'text/markdown' : 'text/plain' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `${(activeConversation?.title || 'adesh-chat').replace(/[^a-z0-9]+/gi, '-').toLowerCase()}.${type === 'md' ? 'md' : 'txt'}`; link.click(); URL.revokeObjectURL(link.href); };
